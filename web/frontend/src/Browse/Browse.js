...
function GenreList() {
    ...
    return <>
        <h1>Genres</h1>
        <div className={styles.list}>{map(Array.from(parsePairs(data)),
            ([_, v]) => <div
                key={v}
                className={styles.list}
            ><Link
                 to={`/web/browse/genre/${encodeURIComponent(v)}`}
            >{v}</Link></div>
        )}</div>
    </>
}
...
function GenrePage(props){
    ...
    const {loaded, err, data} = useMPDQuery(`list artist genre \"${genreName}\"`);
    if (!loaded)
        return <div/>
    if (err) {
        return <h3>Error occurred: {JSON.stringify(error)}</h3>
    ...
    return <>
        ...
        <div className={styles.list}>{map(
            Array.from(parsePairs(data)),
            ([_, v]) => <div><Link to={`/web/artist/${encodeURIComponent(v)}`}>{v}</Link></div>
        )}</div>
    </>
}
